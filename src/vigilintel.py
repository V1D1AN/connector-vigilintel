...
JSON Sections processed:
- Analyse transversale: Strategic analysis
- Synthese des acteurs malveillants: Threat actors  
- Synthese de l'actualite geopolitique: Geopolitical notes
- Synthese des violations de donnees: Data breaches/incidents
- Synthese des vulnerabilites: CVEs
- Articles: Detailed threat reports with IOCs and TTPs
"""
...
class VigilIntelConnector:
    def __init__(self):
        ...
        self.vigilintel_ioc_mapping = {"IPv4": "ipv4", "DOMAIN": "domain", "URL": "url", "SHA256": "sha256", "FILE_HASH_SHA256": "sha256", "MD5": "md5"}
    ...
    def _process_json_threat_actors(self, actors_list: List[Dict], report_date: datetime) -> Tuple[List, List]:
        ...
        for actor in actors_list:
            try:
                ...
                desc_parts = []
                if actor.get("secteur"): desc_parts.append(f"**Sector:** {actor['secteur']}")
                if actor.get("modop"): desc_parts.append(f"**Modus Operandi:** {actor['modop']}")
                labels = [t for t in actor.get("tags", []) if t not in ["actor", "theat_actor", "threat_actor"]]
                ta = ThreatActor(name=name, description="\n\n".join(desc_parts) if desc_parts else None,
                    created_by_ref=self.identity_id, labels=labels if labels else None,
                    external_references=self._create_external_references(actor.get("sources", [])) or None,
                    created=report_date.strftime("%Y-%m-%dT00:00:00Z"), modified=report_date.strftime("%Y-%m-%dT00:00:00Z"))
                stix_objects.append(ta)
                object_refs.append(ta.id)
                self.helper.log_info(f"  ThreatActor: {name}")
            except Exception as e:
                self.helper.log_error(f"ThreatActor error: {str(e)}")
    ...
    def _process_json_vulnerabilities(self, vulns_list: List[Dict], report_date: datetime) -> Tuple[List, List]:
        ...
        for vuln in vulns_list:
            try:
                ...
                cvss, product, desc = vuln.get("cvss"), vuln.get("product", ""), vuln.get("description", "")
                full_desc = desc + (f"\n\n**Product:** {product}" if product else "") + (f"\n\n**CVSS:** {cvss}" if cvss else "")
                ...
                labels = [t for t in vuln.get("tags", []) if t not in ["vulnerabilite", "vulnerability"]]
                v = Vulnerability(name=cve_id.upper(), description=full_desc if full_desc else None,
                    created_by_ref=self.identity_id, labels=labels if labels else None, external_references=ext_refs,
                    created=report_date.strftime("%Y-%m-%dT00:00:00Z"), modified=report_date.strftime("%Y-%m-%dT00:00:00Z"))
                stix_objects.append(v)
                object_refs.append(v.id)
                self.helper.log_info(f"  Vulnerability: {cve_id} (CVSS: {cvss})")
            except Exception as e:
                self.helper.log_error(f"Vulnerability error: {str(e)}")
        return stix_objects, object_refs

    def _process_json_incidents(self, incidents_list: List[Dict], report_date: datetime) -> Tuple[List, List]:
        stix_objects, object_refs = [], []
        for inc in incidents_list:
            try:
                victim = inc.get("victime", "")
                if not victim: continue
                content = f"**Sector:** {inc.get('secteur', '')}\n\n**Details:** {inc.get('description', '')}"
                labels = [t for t in inc.get("tags", []) if t not in ["violation", "breach"]] + ["data-breach"]
                victim_id = Identity(name=victim, identity_class="organization", created_by_ref=self.identity_id)
                stix_objects.append(victim_id)
                note = Note(abstract=f"Data Breach: {victim}", content=content, created_by_ref=self.identity_id,
                    object_refs=[victim_id.id], labels=labels,
                    external_references=self._create_external_references(inc.get("sources", [])) or None,
                    created=report_date.strftime("%Y-%m-%dT00:00:00Z"), modified=report_date.strftime("%Y-%m-%dT00:00:00Z"))
                stix_objects.append(note)
                object_refs.extend([note.id, victim_id.id])
                self.helper.log_info(f"  Incident: {victim}")
            except Exception as e:
                self.helper.log_error(f"Incident error: {str(e)}")
        return stix_objects, object_refs
    ...
    def _process_json_geopolitical(self, geo_list: List[Dict], report_date: datetime) -> Tuple[List, List]:
        ...
        for geo in geo_list:
            try:
                ...
                content = f"**Sector:** {geo.get('secteur', '')}\n\n{geo.get('description', '')}"
                labels = [t for t in geo.get("tags", []) if t not in ["geopolitique", "geopolitic"]] + ["geopolitical-analysis"]
                note = Note(abstract=f"Geopolitical: {theme[:200]}", content=content, created_by_ref=self.identity_id,
                    object_refs=[self.identity_id], labels=labels,
                    external_references=self._create_external_references(geo.get("sources", [])) or None,
                    created=report_date.strftime("%Y-%m-%dT00:00:00Z"), modified=report_date.strftime("%Y-%m-%dT00:00:00Z"))
                stix_objects.append(note)
    ...
    def _process_json_analysis(self, analysis_data: Dict, report_date: datetime) -> Tuple[List, List]:
        stix_objects, object_refs = [], []
        try:
            text = analysis_data.get("analyse", "")
            if text:
                note = Note(abstract=f"Strategic Analysis - {report_date.strftime('%Y-%m-%d')}", content=text,
                    created_by_ref=self.identity_id, object_refs=[self.identity_id], labels=["strategic-analysis", "daily-summary"],
                    created=report_date.strftime("%Y-%m-%dT00:00:00Z"), modified=report_date.strftime("%Y-%m-%dT00:00:00Z"))
                stix_objects.append(note)
                object_refs.append(note.id)
    ...
    def _process_json_articles(self, articles_list: List[Dict], report_date: datetime) -> Tuple[List, List]:
        stix_objects, object_refs = [], []
        for article in articles_list:
            try:
                ...
                analysis = article.get("analyse", "")
                recs = article.get("recommandations", [])
                tags = article.get("tags", [])
                sources = article.get("sources", [])
                iocs = article.get("indicator_of_compromise", {})
                ttps = article.get("mitre_ttps", [])
                
                article_refs = []
                content_parts = []
                if desc: content_parts.append(f"## Description\n{desc}")
                if analysis: content_parts.append(f"## Analysis\n{analysis}")
                if recs:
                ...
                # IOCs
                if self.create_indicators and iocs:
                    for cat, vals in iocs.items():
                        ...
                        for val in vals:
                            ...
                            if ind_data:
                                ind = Indicator(name=f"{ioc_type.upper()}: {ind_data['ioc_value']}", pattern=ind_data["pattern"],
                                    pattern_type="stix", created_by_ref=self.identity_id, labels=tags if tags else None,
                                    external_references=ext_refs if ext_refs else None, valid_from=report_date.strftime("%Y-%m-%dT00:00:00Z"))
                ...
                for ttp in ttps:
                    ...
                    for tech_id in matches:
                        ap = AttackPattern(name=ttp, created_by_ref=self.identity_id,
                ...
                # Note
                note = Note(abstract=title[:250], content=content, created_by_ref=self.identity_id,
                ...
                self.helper.log_info(f"  Article: {title[:50]}... ({len(article_refs)} refs)")
            except Exception as e:
                self.helper.log_error(f"Article error: {str(e)}")
        return stix_objects, object_refs

    def _process_json_full(self, json_data: Dict, work_id: str, report_date: datetime) -> Tuple[int, str]:
        try:
            lang_key = self.language.upper()
            data = json_data.get(lang_key) or json_data.get("FR") or json_data.get("EN")
            if not data:
                return 0, "No language data found"
    ...
    def _process_report(self, report_date: datetime, work_id: str) -> Tuple[bool, str]:
        ...
        count, status = self._process_stix_bundle(data, work_id, report_date) if self.format == "stix" else self._process_json_full(data, work_id, report_date)
